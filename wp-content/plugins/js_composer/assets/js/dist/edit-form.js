...
_.isUndefined(window.vc) && (window.vc = {
    ...
    function($) {
        ...
        media = wp.media, preloader_url = ajaxurl.replace(/admin\-ajax\.php/, "images/wpspin_light.gif"), media.controller.VcCssSingleImage = media.controller.VcSingleImage.extend({
            ...
            setCurrentBgImage: function(value) {
                var url, image_split, template, image_regexp = /([^\?]+)(\?id=\d+){0,1}/,
                    id = "";
            ...
            init: function(param, $field) {
                $("[data-css-editor=true]", this.content()).each(function() {
                    var keys, params, cssString, $editor = $(this),
                    ...
                    }, params = vc.edit_element_block_view.model.get("params"), value = (cssString = _.reduce(keys, function(memo, css_name, attr_name) {
    ...
    function($) {
        ...
        var ParamsPresetView = Backbone.View.extend({
            ...
            setParams: function() {
                var data, fields, val = this.save(),
                    $option = this.$el.find('[value="' + val + '"]'),
                    $edit_form = vc.edit_element_block_view.content(),
                    $params_preset = this.$el;
                $option.length && (data = $option.data("params"), fields = [], _.each(data, function(value, key, list) {
