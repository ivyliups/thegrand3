jQuery(document).ready(function($){
	//upload
	$("[name='medicenter_upload_button']").on('click', function(){
		var self = $(this);
	...
	$("#medicenter_add_new_button").on("click", function(){
	...
	$("#medicenter_add_new_button_image").on("click", function(){
	...
	$(".medicenter_add_new_repeated_row").on("click", function(){
	...
	//departments hours
	$("#add_department_hours").on("click", function(event){
	...
	$(document.body).on("click", "#department_hours_list .delete_button", function(){
	...
	$(document.body).on("click", "#department_hours_list .edit_button", function(){
		if(typeof($(this).parent().attr("id"))!="undefined")
		{
	...
	$("#import_dummy").on("click", function(event){
		...
		$.ajax({
				url: ajaxurl,
				type: 'post',
				data: "action=ql_importer_import_dummy&themename=" + config.themename,
				success: function(json){
					json = $.trim(json);
					...
					$.ajax({
							url: ajaxurl,
							type: 'post',
							data: "action=ql_importer_import_dummy2&themename=" + config.themename,
	...
	$("#slider_delete_button").on("click", function(){
	...
	$(".for_main_color a").on("click", function(event){
		event.preventDefault();
		$("#color_scheme").val($(this).data("color")=="42B3E5" ? "blue" : ($(this).data("color")=="7CBA3D" ? "green" : ($(this).data("color")=="FFA800" ? "orange" : ($(this).data("color")=="F37548" ? "red" : ($(this).data("color")=="00B6CC" ? "turquoise" : ($(this).data("color")=="9187C4" ? "violet" : '')))))).trigger("change");
		$("#primary_color").val($(this).data("color")).keyup();
		$("#primary_color").prev().css("background-color", "#" + $(this).data("color"));
		$("#secondary_color").val($(this).data("color-secondary")).keyup();
		$("#secondary_color").prev().css("background-color", "#" + $(this).data("color-secondary"));
		$("#tertiary_color").val($(this).data("color-tertiary")).keyup();
		$("#tertiary_color").prev().css("background-color", "#" + $(this).data("color-tertiary"));
	});
	//google font subset
	$("#header_font, #content_font, #blockquote_font").change(function(event, param){
		var self = $(this);
		if(self.val()!="")
		{
